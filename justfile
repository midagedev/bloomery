...
#
# 레시피는 절대 `cd ~/repo/mulle`를 쓰지 않는다. box.sh가 이미 $REMOTE로 들어가고 그 값은
# 워크트리 이름에서 유도된다 — 레시피가 다시 cd하면 워크트리 라운드가 메인 트리를 재게 된다
# (2026-09-19 ffn 라운드가 보고: "gate-ffn은 ~/repo/mulle로 가는데 이 워크트리는
# ~/repo/mulle-ffn으로 rsync된다. 둘 다 박스에 있어서 엉뚱한 트리를 시험한다").
...
build-gpu:
    ./tools/box.sh 'cargo oxide build --arch sm_86 -- -p q3k-gemv'
...
gate-ops:
    ./tools/box.sh 'cargo test -p model --test ops -- --ignored --nocapture'

gate-attn:
    ./tools/box.sh 'cargo test -p model --test attn -- --ignored --nocapture'

gate-ffn:
    ./tools/box.sh 'cargo test -p model --test ffn -- --ignored --nocapture'

gate-moe:
    ./tools/box.sh 'cargo test -p model --test moe -- --ignored --nocapture'

gate-head:
    ./tools/box.sh 'cargo test -p model --test head -- --ignored --nocapture'

# 1단계 1-1 게이트: 디퀀트 오라클을 빌드해 ggml의 to_float 덤프를 만들고, gguf 크레이트의
# hw 테스트가 그것과 대조한다. hw_ 접두는 박스를 요구한다는 뜻이고 기본 실행에서 빠져 있다.
gate-1-1:
    ./tools/box.sh 'bash tools/ref/build-dequant.sh && "$MULLE_DATA/bin/dequant_ref" && cargo test -p gguf -- --ignored --nocapture'
