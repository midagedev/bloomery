...
- **보고 받음, 리드 검증 중**: `b4rope`(rope 계열 — 세트 7개·rope 사이트 794개가 ik와 비트 동일, K/V 행 240개가 유도 밴드 27u 안; 리드가 `d1n`을 더했다) ‖ `b2b`(V2-Lite 하이브리드 경계 — n_l 32·0에서 노드 778·700, eager = 재생, 층별 카드 슬롯 비트 동일, 호스트 합 밴드 안; FAIL-first 둘). 둘 다 새 main 위로 리베이스해 게이트를 다시 돌리고, b2b는 A6000에서 기본 경로와 레버 넷(n_l 32·0 × 겹침 켬·끔)을 번갈아 잰다.
...
- 락 없는 호스트 bin 러너 하나(b4plan2 ④-7): `gate-ds41-plan`과 `bench-v41-host`는 `gate.sh && cargo build && timeout <bin>`으로 이어져 종료 코드의 주인이 둘이다 — `tools/gpu-gate.sh`의 락 없는 짝으로(S). `tools/ptx-scan.sh:24-25`의 `router_shape` 설명에 `q8_0_gemv_heads`가 빠졌다(b11c, XS).
