...
- **비행 중(전부 opus)**: 11:07 `b2b`(V2-Lite 하이브리드 경계) ‖ 11:33 `b5plan`(읽기 전용 B5 설계·절단) ‖ 11:37 `b11c`(q8_0 스케일 평면 f16 + V2-Lite 짧은 행 회복 + q8_0 게이트 두 줄 + 배치 재핀) ‖ 11:49 B4 둘째 파동 `b4hc` ‖ `b4rope` ‖ `b4attn` ‖ `b4moe` ‖ `b4plan2`(스펙은 세션 스크래치 `wave-m3/spec-b4*.md`; 11:59 오라클 갱신을 다섯에 알렸다).
