...
- **비행 중(21:30~)**: `b5index`(2단계 — pos ≥ 512의 인덱서 선택을 attention 조각에, d1·d2 세트 G1, c4096 PPL, 스펙 `spec-b5index.md`) ‖ `b5gen`(`generate`가 V4.1 스텝을 몰기 — 실제 스텝으로 깊이, `--time`, ik V4.1 A6000 깊이 러너; 표는 리드가 잰다, 스펙 `spec-b5gen.md`).
- **비행 중(20:30~)**: `cpu3`(프리페치 거리 `BLOOMERY_KV_PREFETCH_ROWS`·`BLOOMERY_SPIN` 레버 팔, 같은 바이너리, 깊이 1024/4096, 스펙 `spec-cpu3.md`) ‖ `cpuresearch`(코드 없음 — 사용자 09-23 "스칼라를 피하고 캐시를 최대한 쓰는 것을 한 설계로 묶을 방법": ik `iqk_flash_attn.cpp` 독해 + 문헌으로 head-안쪽 루프·split-K combine·CCD 고정 키 배분·q8 KV를 우리 형상 수치로, 스펙 `spec-cpuresearch.md`). **결정(리드 추천, 사용자 09-23 "판단은 네 추천대로")**: 깊은 행은 키 분할 + combine(ik의 모양, float 순서 클래스)으로 간다 — 모양은 cpuresearch 뒤에 정한다.
