...
**2026-09-23 새벽(사용자 "적극적으로 병렬 오케스트레이션, 자율 진행")** — main은 파동 10의 b2a 머지(`3663f9a`) 위다. 머지 파동마다 푸시한다(아래 「결정」 ①).
...
- **비행 중(파동 10, 07:41 발사)**: `b1b`(V4.1 적재 — 계획대로 카드별 업로드, 상주 산술의 소유자 하나, 호스트 expert 잠금(선택), `open_backed`가 populate 전에 검증, GPU 적재 게이트 ②; 데이터 `/root/bloomery-data-b1b`) — **08:38 중간 보고**: 게이트 ② (b)안 check 2가 두 카드 다 빨강이다. 할당 반올림이 A6000 284,998,176 B, 3090 264,043,552 B이고(check 1 상주 바이트·check 3 표본 비트는 PASS), 계획의 컨텍스트 항 536,870,912 B를 각각 34,388,512 B·6,290,464 B 넘는다. 리드 결정: 반올림을 여유로 덮지 않고 배치 산술(`model::placement`)의 유도 항으로 넣는다. 2 MiB 규칙 등이 측정을 재현하면 n_l을 재핀하고(`PIN`), 재현하지 못하면 빨강으로 두고 잔차를 보고한다. 반올림을 없애는 카드별 아레나는 B1c ‖ `b4plan`(읽기 전용 — B4를 op별 라운드로 자르고 op마다 덤프 노드·ik CPU 산술·게이트 밴드·5토큰 세트의 구멍을 적는다). ~~B2·B8은 b9h 보고 뒤로 미룬다~~ — b9h가 닫혀 **08:12 ~~`b2a`~~ 발사 → 09:30 머지 `3663f9a`**(캡처된 그래프의 GPU ↔ 호스트 합류 다섯 방식을 한 탐침으로, `bench_join`. A6000에서 일이 없는 왕복은 22–65 µs였다. memop 카운터가 드라이버 스레드 없이 21.5–24.6 µs라 B2b의 합류로 정했다; 장부 카드), 이어 **08:27 ~~`b11`~~ 발사 → 09:06 머지 `9ea91c9`**(B11 — `LANE_UNROLL` 4 → 8, 합산 순서를 지켜 비트 동일; A6000 ABAB에서 V4.1 토큰 그래프 24.74 → 22.58 ms, 유도 −3.2 ms 가운데 −2.16 ms; 장부 카드), 뒤이어 **09:20 `b2h` 발사**(B2의 호스트 다리 벤치 `bench_v41_host` — V4.1 형상, 실제 파일 mmap, 엔진의 디스패치 모양 대 행렬별 디스패치, T 스윕. 유도 대역은 91–136 GB/s로, 호스트 항 (a)가 25–37 ms다. 배치 문서가 가정한 값은 23.08 ms다. `--time`은 리드가 조용한 틈에 돈다. 배치 문서의 tok/s 표는 이 결과와 B11 뒤의 q8_0 대역(615–680 GB/s)을 함께 넣어 한 번에 다시 유도한다), 그리고 **09:24 `b11b` 발사**(B11b — q8_0 m = 1에서 레인 하나가 워드 하나를 읽는다. 합산 순서 클래스이고, 유도 이득은 ≤ 1.7 ms/토큰이다. base 팔 워크트리는 `bloomery-b11b-base`). B8은 b1b 뒤. 테스트 env 라운드는 b1b 머지 뒤(b1b가 V4.1 경로와 기계 입력을 가져간다). engram 임대 측정과 `measure-qdot-rate`는 조용한 틈이 올 때까지 밀린다 — 빌드가 도는 동안 잰 수는 증인이 받지 않는다.
...
| `t_floor` | 작은 격자의 런치 하한: K = 5120에서 q8_0 ~~16.8 µs(격자 64)·20.2 µs(160), f32 16.4 µs(48)~~ **12.0 µs(64)·16.0 µs(160), f32 13.5 µs(48)**, q3_K 5.5–5.8 µs(4–64); K = 4096 q8_0 ~~15.3 µs(128)~~ **11.9 µs**; K = 512 q3_K 1.45 µs(16). 굵은 값은 B11(워프당 8청크 비행) 뒤다 | B9, B11 ABAB(09-23 09:03) | A6000 |
...
| B1c | **카드별 장치 아레나**: 텐서마다 따로 잡는 할당이 2 MiB 단위로 반올림돼 (b)안에서 A6000 285 MB, 3090 264 MB를 버린다(b1b 측정). 한 카드에 큰 할당 하나를 잡고 텐서를 256 B 정렬로 잘라 쓰면 그 몫이 expert 약 17·16개로 돌아온다 [유도: 285 MB ÷ 16,773,120 B/expert]. 디코드 값은 작다 — 토큰당 약 0.05 ms [유도, 접두 규칙]. B12의 뜨거운 집합에서는 슬롯 하나의 값이 커진다. `DeviceTensor`에 행 오프셋 뷰가 없는 것(`tensor.rs:12`)과 같은 자리다 | `crates/gpu/src/tensor.rs`, `weights.rs` 업로드 | opus | 적재 게이트 ②의 check 2가 반올림 항 없이 통과, 전 게이트 비트 동일 | B1b | M |
| B11b | **`q8_0_gemv` m = 1의 남은 몫 — 레인 하나가 워드 하나(코드 넷)를 읽는다**(B11 뒤, A6000). B11의 행보는 레인 넷이 한 워드를 나눠 읽어, 워프의 적재 명령 하나가 32 B다. 큰 런치도 615–680 GB/s에 머물렀다. 같은 카드의 q6_K 헤드는 701, q3_K·q4_K `_sel`은 579–639다. 격자 288인 shared gate/up은 B11에서 0.98배로 거의 움직이지 않았다(510 GB/s). 사슬이 아니라, 요청이 잘게 나뉜 대역 쪽 벽이다. 레인이 워드를 통째로 읽으면 적재 명령 하나가 128 B가 된다. 걸린 몫 ≤ 1.7 ms/토큰이다[유도, 700 GB/s 가정 — q6_K 헤드의 값이지 q8_0의 보장이 아니다]. 내역은 큰·중간 런치 1.3 ms(`attn_output_b` 0.38, shared gate/up 0.56, `attn_q_b` 0.21, shared down 0.14)와 작은 격자 0.4 ms(`attn_kv` 0.27, `attn_q_a` 0.13; 575 GB/s 바닥 기준)다. 묶음 `attn_output_a`는 B4의 한 런치 커널 몫이라 넣지 않았다. 레인마다 합산하는 값이 바뀌므로 **부동소수점 합산 순서 클래스**다 — 오차 모델(σ 진단)과 `gate-gpu-e2e` 개수 핀, `bench_v41` 사이트 줄, A6000 A/B 1회로 증명한다. **열기 전에**: 워드 단위 적재의 레지스터·점유율을 ptxas `-v`로 계산해 스펙에 적는다. 작은 격자에는 split-K(행 하나를 워프 둘 이상이)가 필요한지도 같은 산수로 가른다. B4의 `wo_a` 한 런치 커널과 코어를 나눠 쓸 수 있는지도 본다 | `crates/gpu/src/q8f32.rs` | opus | 오차 모델 + e2e 개수 핀, `bench_v41 --check`, A6000 A/B | B11 | S–M |
...
| B4 | V4.1 고유 op 커널 ×N — op마다 자기 파일·자기 모듈·자기 게이트(결정 6). **B9에서 넘어온 것**: 8그룹 블록 대각 `wo_a`를 한 런치로 도는 커널의 걸린 몫 ~~2.24 ms/토큰~~ **B11 뒤 1.64 ms/토큰**(A6000 ABAB, 묶음 토큰 그래프 22.58 ms 대 밀집 20.93 ms — 같은 바이트 한 런치의 하한이고, 그 커널의 시간은 아니다); `Q8Act`의 k ≤ 10944가 `hc_*_fn`(K = 20480)을 막는다(`tensor.rs:106`); `DeviceTensor`에 행 오프셋 뷰가 없다(`tensor.rs:12`) | `gpu/<op>.rs` + `gate_<op>.rs` 각각 | ~~GLM~~ opus ×N 병렬 | 오라클 v3 밴드(세트 `$BLOOMERY_DATA/ref_deepseek41`, 5토큰 — 인덱서 top-k 노드는 이 길이에서 안 생긴다, `oracle.md`) | B0a, ~~B0c~~(세트 있음, 09-23 06:37) | op당 S~M |
