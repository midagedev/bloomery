...
|---|---|---|---|---|---|
| E1 | **E1a 답함(자리 2)**: 깊이 6 fresh 대 `GEN_WARM=400` — 스텝 p50 ~~42.64~~ 40.0–40.2(같은 창 fresh 프로세스 7개; 42.64는 이상치) → **39.20 ms**(약 +2 %), host leg 26.55 → 25.55(몫 1.0 ms뿐), straggle max 2.83 → 0.48, **majflt 28.5 → 2.5/스텝, minflt 8,109 → 1,076/스텝** — 적재가 expert 페이지를 다 끌어오지 않아 헤드라인(warm 0)이 ~2 % 낮게 읽힘 → 적재 라운드(prefault) 몫 ‖ **E1b 남음**: 실제 텍스트 프롬프트 대 lcg, 깊이 4096(자리 4) | 헤드라인 25.4가 퇴화 몫인지; 깊이 역전이 fault인지 라우팅인지 | 임대 ~15분 | 리드 | — |
...
| E11 | **답 진행(자리 5, 05:48, 계측 켬 팔끼리)**: 뜨거운 목록 hotlist-384 대 접두, A6000, `STEP_STATS=1` — 깊이 6 p50 39.03/36.81 대 접두 43.20(host_slots 203→164, leg p50 26.8→22.5 ms), 깊이 4096 34.89/34.40 대 38.35(slots 197→168, leg 24.6→20.6–21.0), 깊이 1024 39.42/38.54(접두 팔 없음) → **−9…−13 %**. 예측 30–33은 빗나감 — 틀린 항은 카드 적중: in-sample 40–55 %가 lcg 스트림에서는 슬롯 −15…−19 %뿐. **계측 세금 발견**: `STEP_STATS=1` 팔은 같은 창의 끈 팔보다 ~2.5–3 ms 느림(접두 42.6–43.2 대 40.0–40.2) — 프로브(`cuMemGetInfo`/getrusage)가 스텝을 세운다. 계측 켠 수치는 켠 수치끼리만 비교; 자리 7이 끈 팔 + ik 같은 창 | B12 실측; ㊳ ⑪ 프로브 값싸게(XS: vram은 16스텝마다 또는 summary만) | — | 리드 | — |
