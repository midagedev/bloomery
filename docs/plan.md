...
**2026-09-23 새벽(사용자 "적극적으로 병렬 오케스트레이션, 자율 진행")** — main은 파동 9의 rtrace 머지(`6f42658`) 위다. 머지 파동마다 푸시한다(아래 「결정」 ①).
...
- **비행 중(파동 10, 07:41 발사)**: `b1b`(V4.1 적재 — 계획대로 카드별 업로드, 상주 산술의 소유자 하나, 호스트 expert 잠금(선택), `open_backed`가 populate 전에 검증, GPU 적재 게이트 ②; 데이터 `/root/bloomery-data-b1b`) ‖ `b4plan`(읽기 전용 — B4를 op별 라운드로 자르고 op마다 덤프 노드·ik CPU 산술·게이트 밴드·5토큰 세트의 구멍을 적는다). ~~B2·B8은 b9h 보고 뒤로 미룬다~~ — b9h가 닫혀 **08:12 `b2a` 발사**, 이어 **08:27 `b11` 발사**(B11 — `LANE_UNROLL` 4 → 8, 합산 순서를 지켜 비트 동일, 리드 유도 토큰당 ≈ 3.2 ms; 파일은 `q8f32.rs` 하나, base 팔 워크트리 `bloomery-b11-base`)(캡처된 그래프가 호스트 스레드에 일을 넘기고 기다리는 네 방식을 한 탐침으로, `bench_join`; 검사는 3090 게이트 락, 시간은 리드가 A6000에서). B8은 b1b 뒤. 테스트 env 라운드는 b1b 머지 뒤(b1b가 V4.1 경로와 기계 입력을 가져간다). engram 임대 측정과 `measure-qdot-rate`는 조용한 틈이 올 때까지 밀린다 — 빌드가 도는 동안 잰 수는 증인이 받지 않는다.
...
| `BW` (큰 런치) | 567–701 GB/s — 한 런치가 수십 MB면 575 가정이 맞거나 낮다(q8_0 567–598, `q4k_gemv_sel` 579, `q3k_gemv_sel` 619–639, 헤드 q6_K 701) | B9 | A6000 |
| `t_floor` | 작은 격자의 런치 하한: K = 5120에서 q8_0 16.8 µs(격자 64)·20.2 µs(160), f32 16.4 µs(48), q3_K 5.5–5.8 µs(4–64); K = 4096 q8_0 15.3 µs(128); K = 512 q3_K 1.45 µs(16) | B9 | A6000 |
...
| B11 | **비행 중(`b11`, 08:27)** — **`q8_0_gemv`·`f32_gemv` m = 1의 작은 격자 사슬**: B9에서 격자 64·K 5120이 같은데 q8_0은 16.8 µs, q3_K는 5.8 µs였다. 격자가 아니라 행 하나를 걷는 적재 사슬이다(b9h 독해: `q8f32.rs:282` `q8_0_lane_partial_1col`에서 레인 넷이 같은 워드를 읽어, 워프 적재 한 번에 32 B). 걸린 몫 ≈ 1.16 ms/토큰 [유도 — 작은 격자 q8_0·f32 사이트마다 (측정 − w_bytes/575 GB/s) × 런치 수]. **열기 전에** PTX의 적재 폭에서 워프당 비행 중 바이트를 유도하고, 그 사슬이 16.8 µs를 설명하는지 스펙에 적는다. V2-Lite의 파생 Q8_0 경로도 같은 커널이다 | `crates/gpu/src/q8f32.rs` | opus | 합산 순서가 바뀌면 오차 모델 + `gate-gpu-e2e` 개수 핀, `bench_v41` 사이트 줄, A6000 A/B 1회 | B9 | S–M |
| B12 | **상주 expert 집합을 id 접두에서 층마다 뜨거운 집합으로**(B10 결과, rig-log 09-23#v41-router-coverage): 층당 hot-64가 받는 선택 몫이 held-out에서 61–74 %, 코퍼스를 건너면 40–49 %다. 접두 규칙은 16.7 %다. (a)안 기준 [유도, 건넌 목록 40.1–49.4 % 대 접두 16.6 %; 2–39층 expert 읽기 3.824 GB/토큰(B9의 천장과 같다) 중 GPU 몫이 그만큼, 0·1층 0.219 GB는 그대로 호스트]: 호스트 expert 읽기가 토큰당 3.409 GB에서 2.15–2.51 GB로, 호스트 항 23.08 ms가 14.6–17.0 ms로 준다. GPU 쪽 expert 읽기는 0.634 GB에서 1.53–1.89 GB로 늘어 575 GB/s에서 2.7–3.3 ms다(배치 1 디코드, 선택 몫 = VRAM에서 나오는 읽기 몫). 해야 할 것: ① 커널이 읽는 id → 슬롯 재매핑(층마다 384칸 표, `_sel`의 범위 밖 계약을 재매핑 표의 '없음'으로 — 합치는 쪽은 여전히 id로 가린다) ② 로더가 접두 대신 층마다 id 목록을 올린다(B1b의 접두는 목록 0..n_l의 특수한 경우다) ③ 목록의 출처 — 정적 파일(두 코퍼스 합친 목록이 자기 목록보다 3–6포인트만 잃는다)인지 온라인 갱신인지. **열기 전에**: 서빙에 가까운 셋째 코퍼스(채팅·자연어·한국어)의 추적 — 지금 둘은 한 프로젝트에서 나왔다. 5만 토큰에 11–13분이다 | `gpu`의 `_sel` 커널들과 그 런처, `weights.rs` expert 업로드, 새 목록 파일 | opus | 재매핑 없음(항등 목록)에서 지금 게이트 전부 비트 동일, 재매핑 목록에서 `_sel`이 슬롯마다 그 expert의 `q*_gemv`와 비트 동일, 셋째 코퍼스의 건넌 몫 | B1b, B10 | M |
...
| B4 | V4.1 고유 op 커널 ×N — op마다 자기 파일·자기 모듈·자기 게이트(결정 6). **B9에서 넘어온 것**: 8그룹 블록 대각 `wo_a`를 한 런치로 도는 커널의 걸린 몫 2.24 ms/토큰(A6000, 같은 바이트 한 런치의 하한 — 그 커널의 시간은 아니다); `Q8Act`의 k ≤ 10944가 `hc_*_fn`(K = 20480)을 막는다(`tensor.rs:106`); `DeviceTensor`에 행 오프셋 뷰가 없다(`tensor.rs:12`) | `gpu/<op>.rs` + `gate_<op>.rs` 각각 | ~~GLM~~ opus ×N 병렬 | 오라클 v3 밴드(세트 `$BLOOMERY_DATA/ref_deepseek41`, 5토큰 — 인덱서 top-k 노드는 이 길이에서 안 생긴다, `oracle.md`) | B0a, ~~B0c~~(세트 있음, 09-23 06:37) | op당 S~M |
