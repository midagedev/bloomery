...
| E21 | **T1 실측: 3090 단일 배치의 tok/s** — `BLOOMERY_TIMING_GPU=<3090>` `--place gate` + `BLOOMERY_HOT_LIST`, 깊이 6/4096, 코드·산문 512토큰 프롬프트, `-n 96`, 2바퀴(A6000 고정 규칙의 예외 — **사용자 승인 09-24**) | 공개 글 헤드라인 = 타겟 카드의 값; 예측[유도]: 슬롯 888→~600이면 호스트 슬롯 +30 %, 스텝 ~+7–8 ms → 22–24 tok/s(드래프트 없이) | 임대 ~20분 | 리드 | 사용자 승인 |
