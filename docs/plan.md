...
- **비행 중(파동 9, 03:58 발사 — 04:10 리밋으로 셋 다 base 단계에서 죽었고(트리 무변경), 06:04 재개)**: `b1a`(배치 표·Q8_0/BF16·분할 리더, 박스 데이터 `/root/bloomery-data-b1a`) ‖ ~~`b7`~~ 머지(`fddfa5f`, 06:54 — q5_K CPU 경로, 장부 카드) ‖ ~~`m3c`~~ 머지(`c463c73`, 06:44 — 공유 파일이 deepseek2에 닿는 길을 닫고 check-arch ④, 장부 카드) — 전부 opus. 뒤이어 뜬 것: ~~`b6`~~ 머지(`355fcba`, 07:05 — `q4k_gemv_sel`, 장부 카드) ‖ `ptxscan`(07:00 — b6가 찾은 ptx-scan 추출 결함을 oxide-artifacts의 자기 파서로 닫고, 실패한 스캔은 rc ≠ 0; base는 b6) ‖ `b9h`(07:12 — B9의 벤치 하니스: V4.1 한 토큰의 GPU 밀집 읽기 전부 — q8_0 형상, 헤드 q6_K, VRAM expert `_sel` — 층마다 다른 사본으로 L2를 피한다; 기능 확인만 하고 시간은 리드가 임대 아래 A6000에서) ‖ `rtrace`(V4.1 라우터 추적을 프리필로, 06:39 — CPU 임대, GPU 안 씀, 박스 쓰기는 `$BLOOMERY_DATA/router/`만). 공유 `$BLOOMERY_DATA`에 쓰는 레시피(`gate-1-1`·`build-ref`·`build-qdot-ref.sh`)는 트랙 데이터 디렉터리에서만 돈다: 한 트랙이 `bin/dequant_ref`를 다시 짓는 사이 다른 트랙이 옛 바이너리를 부르면 V2-Lite 매니페스트가 V4.1 것으로 덮인다(조언자 지적, 발사 전).
...
| B9 | **하니스 비행 중(`b9h`, 07:12)** — **`q8_0_gemv`를 V4.1 형상에서 잰다**(K = 1280·2304·4096·5120·6144·8192, A6000): 토큰당 GPU 밀집 읽기 8.58 GB 중 7.69 GB를 이 커널이 나르고, ik를 이기느냐가 이 대역폭에 달렸다(v41-placement §5 — 247 GB/s면 16 tok/s, 575면 24–27) | 벤치 하니스에 형상 추가(라운드) → 리드가 임대 아래서 | opus → 리드 | 형상별 GB/s 표(A6000, 위트니스), 유도 대역과 대조 | — | S |
...
| B5 | V4.1 조립 + 첫 토큰 | `gpu/model.rs`, `crates/model` | ~~GLM~~ opus(2026-09-22부터 위임 백엔드) → 리드 | **PPL 2.2355 패리티, tok/s 대 ~~25.05~~ ik 20.4–20.7**(25.05는 DSpark 드래프트를 켠 값 — `roofline.md`에 선 그어 정정; 드래프트 없는 mainline은 17.7). Q5_K가 모델 안에서 처음 타는 자리(0·1층 down, `ops.rs`의 융합 경로)도 여기서 판정된다 — V2-Lite 게이트에는 Q5_K가 없다 | A3, B1~B4 | L |
...
| **9 (2026-09-23 새벽, 03:58 발사 · 04:10 리밋 · 06:04 재개)** | `b1a`(`gguf` 리더·`GgmlType` 구역·`ops.rs` 비융합 분기의 거부, 새 배치 모듈, `arch/deepseek41/`, 데이터 `-b1a`) ‖ ~~`b7`~~(`quant.rs`의 Q5_K 디퀀트, `crates/qdot`, qdot 참조 하니스, 데이터 `-b7`) 머지 `fddfa5f`(06:54) ‖ ~~`m3c`~~(GPU 트랙 하나 — 공유 데이터는 읽기만, `gate-1-1` 안 돎) 머지 `c463c73`(06:44) — 뒤이어 ~~`b6`~~(`q4k_gemv_sel`, 06:26) 머지 `355fcba`(07:05) ‖ `rtrace`(라우터 추적, 06:39, CPU 임대) ‖ `ptxscan`(ptx-scan 추출, 07:00) | `quant.rs`를 세 라운드가 구역으로 나눠 쓴다(B1a는 `GgmlType`과 `dequant_q6_k` 뒤의 Q8_0·BF16 함수, B7은 `dequant_q4_k` 뒤의 Q5_K 함수, M3c는 `half_to_f32` 바로 뒤와 파일 끝) — `dequant_row`의 match에서 B1a·B7의 팔이 이웃하므로 둘째로 머지하는 쪽은 리드가 손으로 합친다. 조용한 틈에 engram 임대 측정. 아침에 사용자 승인 묶음(푸시·B0c 덤프·ik 재빌드·v41-placement §8의 넷) | B1a·B7·M3c 머지, `check-arch` ①이 `arch/deepseek41/`에서 값을 받기 시작 |
