...
- **비행 중(09-24 01:15~, 전부 opus, base `1260aab`)**: `cpu5`(연구 메모 (d) — 세그먼트 안 8head×1key KQ 타일·8head×1d V 타일, 변환 재사용 8배, 레버 `BLOOMERY_ATTN_BUNDLE`, 스펙 `spec-cpu5.md`) ‖ `b5deep`(`generate_ds41`의 pos ≥ 512 거부 해제 — b5index 뒤로는 낡은 검사; eager = graph 토큰 @1024 증명, 스펙 `spec-b5deep.md`) ‖ `ikload`(ik V4.1 GPU 적재 실패의 갈림 — `-ngl 0/1/999`·`--n-cpu-moe` 대 `-ot`, 로더 file:line, 적재되는 플래그로 깊이 6 ik 행, 스펙 `spec-ikload.md`). 리드 몫: 세 보고 → 머지 → V4.1 깊이 표(6/1024/4096, ik 열 포함) → 아침 보고.
