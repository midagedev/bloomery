...
|---|---|---|---|
| **8′ (2026-09-23 새벽)** | `m3`(`gpu-gates`·`AnyEngine`·`DEFAULT_MODEL` → 프로필) ‖ ~~`m3b`~~(`weights.rs`·`model.rs`·`arch/deepseek2`) 머지 `c179548` ‖ ~~`b3e`~~(`crates/engram`) 머지 `bd4bcd6` ‖ `b0c`(`tools/ref` 덤퍼·프로필, 박스 쓰기는 `/root/bloomery-data-b0c`만) ‖ `cudares`(조사, 파일 무변경) ‖ `b1d`(B1 배치 설계 유도, 파일 무변경) — 전부 opus | CUDA 13.3 전환(끝남 — 「지금」), plan.md 분할, 세 머지 → `check-arch` 엄격 전환 → B3b·B3e 임대 측정 | 세 머지, `check-arch` 엄격 초록 |
