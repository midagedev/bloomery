...
- **사용자 판단 대기**: bloomery main 푸시, ~~B0c 오라클 v3 덤프(RAM 250 GB·두 카드, 30분 초과)~~(승인 항목이 아니게 됐다 — CPU 백엔드만 쓰고 유도 1.5–8분이다. 리드가 조용한 틈에 `BLOOMERY_MODEL=deepseek41 ./tools/box.sh 'bash tools/ref/dump.sh'`로 돈다. NVMe ~477 GB 읽기, page cache ~245 GiB가 뒤집힌다), ik를 13.3으로 다시 빌드할지(빌드하면 ik 기준선을 다시 재야 한다). v41-placement.md §8의 넷: 3090으로 서빙하는가(+10.6 %, 대신 게이트·빌드 카드를 잃는다), 서빙 ctx_max 32k 대 1M(KV 110 MB → 3.36 GB, −0.9 %), 호스트 RAM 203 GiB를 expert용으로 잠그는가, 라우터 id 추적 ~1.4시간(30분 초과).
