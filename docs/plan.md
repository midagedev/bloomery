...
| E3 | `STEAL_BLOCKS` 4/16/64 (bench_v41_host, 프로세스마다 다른 창이라 A/A 포함, 1 % 미만은 판정 불가 — host-rate.sh에 env 팔 루프가 없다, S) ‖ `BLOOMERY_HYBRID_OVERLAP=0`은 GPU 레버라 `depth-ds41` 팔 | ㊴ P2 꼬리 상한 2.2 ms ‖ ㊵ Q3 | 임대 ~4분 ‖ ~10분 | 리드 | probes |
| E4 | mmap 대 익명 THP 순수 읽기 16·32스레드 — probes가 `read-mmap`/`read-thp` 팔을 넣음(`accc23e`). **배관 실행(판정 아님)에서 이미 방향이 보인다**: read-mmap 140.3 ≈ read-thp 141.2 > engine 130.4 GB/s → 틈은 매핑이 아니라 커널·claim·양자화 몫; 읽기 141 대 STREAM 147.7의 차는 정적 분할 풀의 자체 상한[유도]. THP 비율이 실행마다 65–94 %(defrag madvise, `MADV_COLLAPSE` ENOMEM) — summary의 `anon_huge_kb`를 먼저 읽고 팔을 믿을지 정한다 | ㊴ A: 3.47 ms 틈이 TLB인지 디스패치인지 | 임대 ~4분(E2와 한 줄: `just time-cpu-v41-host --threads 16,32 --rounds 6 --seconds 6 --arms read-mmap:5,read-thp:5,engine:5,read-mmap:6,read-thp:6,engine:6`) | 리드 | probes 머지 |
...
| E8 | VRAM margin: `stat` 줄의 `vram_free`/summary `vram_free_min`(probes). 3090 plan_gate 배관 실행 5스텝 전부 1,404,895,232 B 불변(예측대로 재생은 할당 0) — A6000 (a)에서 1,001스텝: `BLOOMERY_CARD=a6000 BLOOMERY_BOX_ENV='BLOOMERY_STEP_STATS=1' just gen-ds41 --place a --depth 6 -n 1001` (임대 없음, ~1–2분) | ㊳ C4: margin 1 GiB 중 실제 쓰는 몫 → n_l 재핀(−0.18 ms 상한) | ~2분 | 리드 | probes 머지 |
...
| E15 | q8dead 뒤 m=1 `q8_0_gemv` 바이너리 A/B (행이 바뀐 경우만) | 죽은 갈래 제거가 디코드 시간을 바꾸는지(≤0 예측) | 임대 ~10분 | 리드 | q8dead |
