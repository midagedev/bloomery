...
- **비행 중(전부 opus, 17:30~)**: `b4-index`(op F: 인덱서 쿼리·가중치·점수·top-k, 목록은 행 오름차순) ‖ `b5attn`(attention 조각, 1단계) ‖ `b5ffn`(MoE 조각과 호스트 티어 합류 — V4.1 호스트 서비스) ‖ ~~`b5glue`~~(머지 `e757bd1`: glue 조각·G1 게이트·b5load 후속 셋, 핀 8개 FAIL-first). 스펙은 세션 스크래치 `wave-m4/`(`common.md`·`chain.md`·`spec-*.md`). 조각의 계약은 `chain/mod.rs`에 있다(prep `326b2a2`): 적재 때 한 번 짓고, enqueue는 발사만 하고, 공유 버퍼와 `Weights`를 인자로 받는다 — `Body`는 받지 않는다. 게이트는 `Weights::load_where`로 층 하나만 올린다. 17:40부터 둘 더: ~~`harness3`~~(머지 `b36f131`: 열셋 전부, 판정 줄·ptx-scan 14표 base와 동일, lint 169) ‖ `idxkey3`(#2507 변형 둘의 c4096 PPL — fp4 반올림, 스트림 분리; 사용자 승인 "둘다 진행해줘", CPU 약 45분).
