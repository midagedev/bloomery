...
| A3f | 같은 f16 레버를 Q4_K에(두 배 크기) + 죽은 mov + 주소 강도 감소 — `q4kdec` | Q4_K 최대 팔 702.5 GB/s 기준 전후, 스텝 p50 |
| A3g | 런치 쪽 2 — `flash_merge`의 q8_1 side output(~44 µs), MoE 양자화 둘 병합(~36 µs), **grid barrier 가격 실측** — `fmerge` | 노드 수, 스텝 p50, 그리고 barrier µs/런치 대 노드 0.80 µs |
