...
pp512의 사다리는 91 → ~~127(1) → 146(1 + h1fold)~~ 정정 09-25: 1은 pp512에 거의 0(위), h1fold + unionreal + CED로 ≈ 107–112[유도, 시팅 13] → h3tile → 호스트 바닥 ≈ 40층 × (317 × 126 µs) = 1.6 s → **≈ 320**(DRAM 바닥, 카드가 그늘에 든 뒤). pp4096은 89 → ~~CED·스트리밍 분담·어긋내기가 곱으로 붙어~~ CED로 152.8[시팅 13] → ds41bulk(194–211[유도]) → ~~Part B 어긋내기(219–229)~~ 층 우선 G 스케줄러 step ①이 같은 겹침을 주므로 Part B는 그 안에 든다 → 스트리밍 분담(~~420–556은 균등 가정, 실측 곡선으로 재유도 중~~ lcg 417–556, prose 512–753[유도, hoststream-recal]).
