...
- **비행 중(파동 9, 03:58 발사 — 04:10 리밋으로 셋 다 base 단계에서 죽었고(트리 무변경), 06:04 재개)**: `b1a`(배치 표·Q8_0/BF16·분할 리더, 박스 데이터 `/root/bloomery-data-b1a`) ‖ ~~`b7`~~ 머지(`fddfa5f`, 06:54 — q5_K CPU 경로, 장부 카드) ‖ ~~`m3c`~~ 머지(`c463c73`, 06:44 — 공유 파일이 deepseek2에 닿는 길을 닫고 check-arch ④, 장부 카드) — 전부 opus. 뒤이어 뜬 것: ~~`b6`~~ 머지(`355fcba`, 07:05 — `q4k_gemv_sel`, 장부 카드) ‖ `ptxscan`(07:00 — b6가 찾은 ptx-scan 추출 결함을 oxide-artifacts의 자기 파서로 닫고, 실패한 스캔은 rc ≠ 0; base는 b6) ‖ `rtrace`(V4.1 라우터 추적을 프리필로, 06:39 — CPU 임대, GPU 안 씀, 박스 쓰기는 `$BLOOMERY_DATA/router/`만). 공유 `$BLOOMERY_DATA`에 쓰는 레시피(`gate-1-1`·`build-ref`·`build-qdot-ref.sh`)는 트랙 데이터 디렉터리에서만 돈다: 한 트랙이 `bin/dequant_ref`를 다시 짓는 사이 다른 트랙이 옛 바이너리를 부르면 V2-Lite 매니페스트가 V4.1 것으로 덮인다(조언자 지적, 발사 전).
