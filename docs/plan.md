...
| 1-5 스레드 | `matmul_q` 행 병렬(`crates/threads` 상주 풀) + 토큰 무관 가중치 준비 캐시(`Derived`) | 스레드 수와 무관하게 로짓 **비트 동일**, 스윕 1/8/16/32/64. `just gate-mt`, `just gate-derived`, `just measure-sweep` | 1-5 프로파일 |
