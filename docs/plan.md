...
|---|---|---|---|---|---|
| E1 | **E1a 답함(자리 2)**: 깊이 6 fresh 대 `GEN_WARM=400` — 스텝 p50 42.64 → **39.20 ms**(tok/s(p50) 23.45 → 25.51, +8 %), host leg 26.55 → 25.55(몫 1.0 ms뿐), straggle max 2.83 → 0.48, **majflt 28.5 → 2.5/스텝, minflt 8,109 → 1,076/스텝** — 적재가 expert 페이지를 다 끌어오지 않아 헤드라인(warm 0)이 8 % 낮게 읽힘 → 적재 라운드(prefault) 몫 ‖ **E1b 남음**: 실제 텍스트 프롬프트 대 lcg, 깊이 4096(자리 4) | 헤드라인 25.4가 퇴화 몫인지; 깊이 역전이 fault인지 라우팅인지 | 임대 ~15분 | 리드 | — |
