...
**2026-09-23 새벽(사용자 "적극적으로 병렬 오케스트레이션, 자율 진행")** — main `e3cf0dc`, origin보다 38커밋 앞(푸시는 사용자 승인).
...
- **비행 중(파동 9, 03:58 발사 — 04:10 리밋으로 셋 다 base 단계에서 죽었고(트리 무변경), 06:04 재개)**: `b1a`(배치 표·Q8_0/BF16·분할 리더, 박스 데이터 `/root/bloomery-data-b1a`) ‖ `b7`(q5_K CPU, `/root/bloomery-data-b7`) ‖ `m3c`(공유 파일이 deepseek2에 닿는 길 닫기 + check-arch ④ — 이번 파동의 유일한 GPU 트랙, 공유 데이터는 읽기만) — 전부 opus. 공유 `$BLOOMERY_DATA`에 쓰는 레시피(`gate-1-1`·`build-ref`·`build-qdot-ref.sh`)는 트랙 데이터 디렉터리에서만 돈다: 한 트랙이 `bin/dequant_ref`를 다시 짓는 사이 다른 트랙이 옛 바이너리를 부르면 V2-Lite 매니페스트가 V4.1 것으로 덮인다(조언자 지적, 발사 전).
