...
- **비행 중(09-24 08:08~, base `4b6c0e0`)**: `fixheld`(보류 7건 44·45·46·89·91·121·122 — gate_load_v41 check 3 출력, hybrid/model.rs "[0, n_l)" 문서, plan 줄 `hot_list=`, `Boundary.hsum` 이중 창 제거, `logits()` pair 뒤 행 A 문서, `history.capacity()`→ctx_max, `fed > a.ctx`→plan ctx_max; 스펙 `spec-fixheld.md`) ‖ `chat`(M2 첫 바인딩 — `gpu-gates::generate::Generator`(open/prefill/logits/step) + bin `bloomery-chat`(텍스트 in → 스트리밍 텍스트 out, 샘플링 플래그, `--greedy`), 게이트 `gate-gpu-ds41-chat` = greedy ids가 `generate_ds41 --tokens`와 동일 + 디코드 자기일치 + 같은 시드 동일; 채팅 템플릿은 serve 몫; 스펙 `spec-chat.md`).
- **비행 중(09-24 08:20~, 조사)**: `pain`(사용자 09-24 "호환은 하되 기존 사용자들의 페인포인트 조사해서 개선" — llama-server·llama-cli·ik 서버·ollama 사용자의 반복 불편 20–40건을 출처 URL·날짜·열림/닫힘과 함께 6묶음(적재·서빙·투기/MoE·CLI·관측·클라이언트 호환)으로, 호환을 깨지 않는 개선 후보 ≤10 순위 + 게이트 → `docs/research/server-painpoints.md`; 박스·코드 없음, 스펙 `spec-pain.md`).
- **비행 중(09-24 08:05~, M4 조사)**: `models`(GLM Flash·Qwen3·V4 Flash의 config/GGUF 메타 표 + 우리 엔진 대비 델타 표 + `arch/` 트레이트 경계 → `docs/research/models-survey.md`; 박스·코드 없음, 출처 URL 필수, 스펙 `spec-models.md`).
...
- **M4 다른 모델 계열 — 대상은 GLM Flash와 Qwen 둘뿐(사용자 09-24 "gpt oss 120b 같은 건 안 해도 돼, 목표는 glm flash와 qwen")**(사용자 09-24 "glm-5.3 flash나 qwen 같은 계열 모델의 지원도 추가하고 싶어"; "qwen 계열이 vram에 통짜로 들어갔을 때 llama보다 빠른 것도 증명하고 싶어")**: M2 뒤. 두 갈래 — ① 카드에 안 들어가는 큰 MoE(GLM-4.x 355B-A32B, Qwen3-235B-A22B): 호스트 티어·핫리스트·드래프트 그대로, 새로 필요한 것은 GQA 플래시 디코드(RoPE 전체·QK-norm)와 라우터 분기(softmax top-k, 공유 expert 없음). GLM-4.x는 라우터가 DeepSeek식이라 먼저. ② **카드에 통째로 들어가는 모델(Qwen3-30B-A3B, Qwen3-8B/32B dense)**: 전 카드 경로에서 llama.cpp/ik와 같은 창 비교 — 근거는 V2-Lite 전 카드 229.5 대 205.5(09-22, A6000); dense는 gemv 효율 싸움이라 폭이 좁고 그래프 캡처·런치 수·헤드 융합이 몫. 첫 단계는 조사 라운드(박스 없음): 후보 체크포인트의 GGUF `general.architecture`·hparams·라우터 식·pre-tokenizer 타입 표(GLM-5.x Flash 구조는 미확인 — 직접 읽어 확정), `arch/`에서 트레이트로 뽑을 경계(어텐션·라우터·norm), 게이트는 지금처럼 `tools/ref/` ggml 하네스 덤프. ③ **DeepSeek V4 Flash**(사용자 09-24 "dgx spark 사용자를 위한 deepseek v4 flash도 대상"): 우리 박스(sm_86 + AVX2 호스트 티어)에서 돌게 하는 것까지가 우리 몫이고, Spark 환경(GB10 sm_121, arm64 호스트, 통합 메모리 128 GB)에 맞는 이식은 **컨트리뷰터를 구한다**(사용자 09-24 "우리 환경에서의 구현만 하고 그쪽 환경에 맞는 구현은 컨트리뷰터를 구하려고") — 공개 글의 "help wanted" 항목. V4 Flash는 V4.1과 같은 계열이라 engram·indexer 유무와 hparams 차이를 GGUF 메타에서 확정하는 것이 조사 라운드의 몫. 공개 순서는 V4.1 헤드라인 먼저, 다중 모델은 그 뒤.
