...
- **다음(리드)**: 재덤프 → 새 세트로 V4.1 게이트 전부 재실행(파동의 base; iqk 층의 수치는 바뀌고 판정은 초록이어야 한다 — attn의 T=1 층은 이제 헤드마다 자기 sink) → 파동 발사. 그다음 `b5step` → `b5index` → `b5time`. 띄울 것(막힌 것 없음): 테스트 env ①b, gates3b(③의 남은 것), harness3(①의 사본 잔여 — harness2가 남긴 것 포함), 32행 아래 잔차 탐침(⑨), 코드 모양 ⑧. **IOMMU pt(사용자 승인 "둘다 진행해줘", 09-23 저녁)**: 박스 GRUB에 `iommu=pt`를 넣어 두었다(`pci=realloc=off iommu=pt`, 원본 `/etc/default/grub.bak-20260923`; 커널은 6.8.0-139 하나뿐이고 nvidia 615.71.09·nvidia-fs 2.30.2 DKMS가 그 커널에 있다, `nvidia-fs`는 modules-load, `gpu-power-limit`·`nvidia-persistenced`는 enabled). 재부팅은 박스 작업이 다 끝난 조용한 때에 하고, 뒤에 `gdscheck -p`와 카드 둘·서비스를 확인한다. 이 뒤의 타이밍은 재부팅 전 수치와 한 표에 두지 않는다(DMA 번역 모드가 바뀐다). 사용자 판단 대기: `attn_output`을 더 낮은 비트로 둘지(토큰당 3.21 GB로 그래프의 약 26 %, 유도 — KLD로 값을 매긴 뒤). 조용한 틈에: 하이브리드 재측정과 nsys M1, engram 임대 측정, `measure-qdot-rate`. `/home/user/ik-sinks{,-base}`와 `/root/bloomery-data-iksink{,-base}`(각 3.5 G), `/root/bloomery-data-idxkey2`, `/home/user/ik-pplnan`(#2513)은 그 PR들이 정리될 때까지 둔다. 지켜볼 것: ik#2511·#2507, cuda-oxide #1314.
...
- b5glue(b5load): `weights.rs:143` `Weights::load`가 `&Gguf`만 받아 `one_shard`가 필요하다 — `Split`으로(S). `weights.rs:318`·`body.rs:68-79,395`에서 `PlacementError`·`PlanError`가 문자열 `Shape`가 되어 source 사슬이 끊긴다(S). Hparams → classify → KvLayout → plan → violations 파이프라인이 세 벌이다(`body.rs:68-88`, `gate_load_v41.rs:145-150`, `gate_deepseek41_load.rs:87-89`) — 주인 하나로(S–M). 돌려 보지 않은 것: `AnyEngine`의 V4.1 팔(컴파일만), `Body::reset`, 다장 카드 계획(`body::open`이 거부한다).
