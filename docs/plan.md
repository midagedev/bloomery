...
- **비행 중**: `seamb`(라우터 const-generic 코어, base `a5f7214`) ‖ `bind`(서버 실 바인딩 + `Tokenizer` 트레이트 + 크래시 맥락, base `bd42fc3`) ‖ `dshc`(드래프트 f32 HC_PRE + bf16 Markov 커널, base `a0444de`). 대기: `dsmx`(seamb 착륙 뒤). 스펙은 세션 스크래치 `wave-m4/`와 `~/.claude/projects/-Users-hckim-repo-bloomery/specs/wave-m4/`(사본).
...
| **M2 돌려 볼 수 있게** | `bloomery-serve-ds41` + `bloomery-chat` + README | `bind`(비행 중) | 머지, BUILD.md 서버 절 검수, **LICENSE 결정 받기** |
...
|---|---|---|---|
| **16 (지금, 09-24 09:10~)** | `seamb` ‖ `bind` ‖ `dshc` — 자리 하나는 비워 둔다(박스 게이트 락이 붐빔) | seama 머지(끝), 이 절, `spec-dsmx`(끝), 트래커 표 갱신 | 셋 머지 |
| 17 | `dsmx`(seamb 뒤) ‖ `qwen3`(장기 시작: Qwen3-30B-A3B 전 카드 — 파일 `arch/qwen3/`, 기존 arch는 읽기만) ‖ `fixup`(트래커 S 이하 10–15건) ‖ `dsgraph`(dshc·dsmx 착륙 뒤 자리 대체) | **측정 자리 8·9 + E21/E21b + E20** — 16과 17 사이 조용한 창(빌더 0, ~1시간) | dsmx·fixup·dsgraph 머지 |
