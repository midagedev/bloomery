...
**2026-09-23 새벽(사용자 "적극적으로 병렬 오케스트레이션, 자율 진행")** — main은 파동 9의 m3c 머지(`c463c73`) 위다. 머지 파동마다 푸시한다(아래 「결정」 ①).
...
| id | 라운드 | 파일 경계 | 백엔드 | 게이트 | 앞 | 크기 |
|---|---|---|---|---|---|---|
| B1a | **비행 중(파동 9)** — **배치 표, 호스트만**([`v41-placement.md`](v41-placement.md) §5·§7①): 분할 인식 GGUF 리더(샤드 9개를 한 인벤토리로 — 지금 리더는 한 파일이고 태그 8·30을 여는 순간 거부한다, `gguf/src/lib.rs:164-240`)와 `GgmlType`의 Q8_0·BF16 팔; 공유 배치 서비스(장치 용량·역할·expert 규칙 → 텐서마다 (장치, 형식, expert 범위)) + deepseek41의 역할 표는 `arch/deepseek41/`(이 라운드가 그 디렉터리를 처음 만든다 — check-arch ①이 값을 받기 시작한다); 표 인쇄 | `crates/gguf/src/lib.rs`, `quant.rs`의 `GgmlType` 구역과 Q8_0·BF16 디퀀트 팔, 새 공유 배치 모듈, `crates/model/src/arch/deepseek41/`, 새 호스트 게이트 | opus | ① 호스트 전용 게이트: 텐서마다 한 번, 층마다 VRAM + 호스트 expert 범위가 0..384를 겹침 없이 덮음, 장치별 합이 설계와 바이트까지 같음((a) A6000 8,576,674,240 + 40,574,177,280, 호스트 218,193,408,000; (b) §5의 세 합), 용량 − 여유 이하, 타입별 형식. FAIL-first: n_l 하나를 +1 → 빨강 | B1d(설계) | M |
| B1b | **적재**: Q8_0 파일 텐서 → 평면(`q8_0_planes`), bf16 → f32 디코드(또는 B8), expert 스택의 행 조각 업로드(지금은 텐서 통째만, `weights.rs:143-152`), `resident_size`를 업로드 산수와 일치(v41-placement §2), 호스트 expert는 샤드 mmap을 잠가서(`mlock`). `Gguf::open_backed`는 헤더를 검증하기 전에 populate한다 — V4.1 첫 샤드에서 7 s(m3 FAIL-first) — 게으른 map으로 파싱한 뒤 populate하게 바꾼다 | `gpu/weights.rs`, 배치 서비스의 장치 쪽, 새 GPU 적재 게이트 | opus | ② GPU 적재 게이트(A6000에 타이밍 실행이 없을 때, 업로드 ~49 GB, 30분 한참 아래): 장치별 `resident_bytes()` = ①의 표, `cudaMemGetInfo` free ≥ 여유(컨텍스트 오버헤드의 첫 측정), `VmLck` = 호스트 표, `mincore` 100 %, 형식별 표본 되읽기 | B1a | M |
...
| B6 | **비행 중(06:26 발사, m3c 다음 GPU 트랙)** — **`q4k_gemv_sel`**: 장치 버퍼의 id로 expert를 고르는 q4_K gemv(`q3k_gemv_sel`·`q5_0_gemv_sel`의 형제). V4.1은 38층의 down이 q4_K라 VRAM expert가 이것 없이 down을 못 돈다(v41-placement §6) | 새 커널 파일 + 새 게이트 | opus | 슬롯마다 그 expert 하나로 돈 `q4k_gemv`와 **비트 동일**(`gate_p9`의 계약), 범위 밖 id는 슬롯을 건드리지 않음, eager = 캡처 | — | S~M |
| B7 | **비행 중(파동 9)** — **q5_K를 CPU에서**: `dequant_q5_k`(ggml `dequantize_row_q5_K` 그대로) + qdot Q5_K×q8_2_x4 융합 커널(활성값 형식은 이미 있다, `quant.rs:548`). 0·1층 down 둘(6.2 GB)이 호스트에 있고 토큰당 ~97.3 MB를 읽는다(유도) | `quant.rs`의 Q5_K 디퀀트 팔과 새 함수, `crates/qdot/**`, ik 참조 하니스(`tools/ref/build-qdot-ref.sh` 쪽) | opus | 디퀀트가 ggml `to_float`와 비트 동일, 커널이 ik 점곱 참조와 qdot의 기존 밴드 안(`gate-qdot` 방식), 커널률을 같은 임대의 ik와 대조(리드) | — | M |
...
| B10 | **비행 중(06:39 발사)** — **라우터의 뜨거움을 잰다(프리필 추적)**: ik 하니스 `router_trace.cpp`가 `ffn_moe_topk-<층>`을 잡아 층별 expert 선택 수를 센다. 먼저 오라클 5토큰으로 40층 전부 정수 일치를 보고, 2,048토큰으로 벽시간을 잰 뒤 30분 안이면 코드·산문 코퍼스 각 ~5만 토큰을 돈다. 산출은 층마다 뜨거운 n개(16–192)가 받는 몫 대 균등 n/384, 해시 라우팅 층과의 차이, 두 코퍼스 사이 hot-64 겹침이다 — v41-placement §9의 "라우터의 뜨거움"이고 상주 집합을 고르는 입력이다(사용자 위임 결정 ③, 프리필 먼저) | `tools/ref/router_trace.cpp`·`router-trace.sh`·`router-coverage.py`, justfile 한 줄, 박스 `$BLOOMERY_DATA/router/`만 | opus | 오라클 세트 `ffn_moe_topk` 정수 사본과 40층 일치 | B0c | S |
...
| **9 (2026-09-23 새벽, 03:58 발사 · 04:10 리밋 · 06:04 재개)** | `b1a`(`gguf` 리더·`GgmlType` 구역·`ops.rs` 비융합 분기의 거부, 새 배치 모듈, `arch/deepseek41/`, 데이터 `-b1a`) ‖ `b7`(`quant.rs`의 Q5_K 디퀀트, `crates/qdot`, qdot 참조 하니스, 데이터 `-b7`) ‖ ~~`m3c`~~(GPU 트랙 하나 — 공유 데이터는 읽기만, `gate-1-1` 안 돎) 머지 `c463c73`(06:44) — 뒤이어 `b6`(`q4k_gemv_sel`, 06:26) ‖ `rtrace`(라우터 추적, 06:39, CPU 임대) | `quant.rs`를 세 라운드가 구역으로 나눠 쓴다(B1a는 `GgmlType`과 `dequant_q6_k` 뒤의 Q8_0·BF16 함수, B7은 `dequant_q4_k` 뒤의 Q5_K 함수, M3c는 `half_to_f32` 바로 뒤와 파일 끝) — `dequant_row`의 match에서 B1a·B7의 팔이 이웃하므로 둘째로 머지하는 쪽은 리드가 손으로 합친다. 조용한 틈에 engram 임대 측정. 아침에 사용자 승인 묶음(푸시·B0c 덤프·ik 재빌드·v41-placement §8의 넷) | B1a·B7·M3c 머지, `check-arch` ①이 `arch/deepseek41/`에서 값을 받기 시작 |
...
- **파동 9의 항목 6 트리아지(2026-09-23 아침)**. *m3c* — 지금 닫은 것: ④의 사각(모듈을 통째로 들이는 `use`), `Q8Block`의 코드 범위 문서, AGENTS.md의 AVX2 문장, 공유 산문 셋(`a59a680`). 테스트 env 라운드로: `ref_dir`가 `Arch`를 받는다. *트래커행*: rustdoc 경고 28건에 래칫이 없다(`attn.rs:563`·`:1504`·`forward.rs:62`·`plan.rs:5`의 중복 명시 링크, gguf·qdot 문서의 `[0]`·`[32]` 미해결 링크 — S–M); base/after 판정 비교 스크립트(시간·스레드 id·pid를 가리고 정렬해 비교 — 세션 스크래치 `m3c/run-set.sh`·`compare.sh`)를 `tools/`로(이동 클래스 라운드마다 손으로 하던 일이다 — S); `dispatch.rs:81` 문서 줄 96열(미관).
