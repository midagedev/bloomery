...
- **비행 중(전부 opus)**: 11:49 B4 둘째 파동 `b4hc` ‖ `b4attn` ‖ `b4moe`(스펙은 세션 스크래치 `wave-m3/spec-b4*.md`; 11:59 오라클 갱신을 알렸다) ‖ B5 첫 둘 `b5meta` ‖ `b5ppl`(「B5 라운드」) ‖ `b4engram`(키 쪽과 쿼리 쪽을 둘로 — wkv와 키 norm은 0층 그늘, 쿼리 쪽은 1·14층 임계) ‖ 13:30 `b4woa`(묶음 `wo_a`를 `q8_0_gemv_heads` 한 런치로) ‖ `harness`(「GPU 선 트리아지」 ①의 하니스 몫 — 매니페스트 한 번 읽기·색인·헤더·f16 마스크 행·스텝 세트 표; b4rope 끝 `1e7e3b9`에서 떴다) ‖ 13:35 `b4comp`(압축기 D + 인덱스 키 E — 선행 셋이 다 풀렸다: infra, rope, #2455 결정; B5 1단계도 pos 0부터 필요하다) ‖ `tools2`(「GPU 선 트리아지」 ② — lease·witness 주인 하나, `prompt()` 한 벌, 락 없는 호스트 bin 러너, `ptx-scan` 피처 인자, 리드가 손으로 짓던 판정 비교·`sass_inflight`·두 트리 A/B를 상설 도구로; `ik-ppl.sh`는 b5ppl 몫이라 뺐다).
- **보고 받음, 리드 검증 중**: `b2b`(V2-Lite 하이브리드 경계 — n_l 32·0에서 노드 778·700, eager = 재생, 층별 카드 슬롯 비트 동일, 호스트 합 밴드 안; FAIL-first 둘). 새 main 위로 리베이스해 게이트를 다시 돌리고, A6000에서 기본 경로와 레버 넷(n_l 32·0 × 겹침 켬·끔)을 번갈아 잰다.
...
- **다음(리드)**: 둘째 파동 보고를 받는 대로 검수·머지한다. b4rope → b2b 순으로 머지(b2b의 새 레버는 AGENTS.md 런타임 레버에 넣었다). **b2b와 b5meta가 머지되면 `b5load` ‖ `b5host`** — 둘 다 b5meta의 `Hparams`·이름 표 위에 선다(b5host는 클램프 한계와 expert 수, b5load는 body 뼈대의 모양). ~~1단계용 스텝 세트 `d1n` 덤프~~ 떴다(`d90de1a`, 876 M). 테스트 env ①의 나머지(①b — 경로 사본 여섯·`ref_dir`의 `Arch`)는 harness 뒤. 도구 ②는 b2b의 A6000 A/B 뒤(A/B에 쓸 측정 러너를 만진다). 테스트 env 라운드와 도구 라운드(「GPU 선 트리아지」 ①·②)는 둘째 파동과 파일이 겹치지 않으면 띄운다. 사용자 판단 대기: IOMMU를 pt로 바꾸는 재부팅(GDS의 다음 단계). 조용한 틈에 engram 임대 측정과 `measure-qdot-rate`.
...
- engram(b4plan2): `crates/engram/src/lib.rs:35-38`의 "엄격한 리더가 이 샤드를 거부한다"는 낡았다(`GgmlType::Q8_0`이 있고 `Split::open`이 V4.1 세트를 연다) — 이 크레이트가 자기 매핑을 갖는 이유를 다시 적는다(XS). ik의 blocked n-gram 규칙이 plan 게이트의 `engram()` 안에만 있다 — `Hash::map_window`로 engram 크레이트가 소유하게(~25줄). `engram.max_ngram_size`를 읽는 곳이 `Hash::from_gguf`와 `Planner::from_file` 둘이다(주인은 b5meta가 정한다).
