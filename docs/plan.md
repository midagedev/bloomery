...
**2026-09-23 낮(사용자 "적극적으로 병렬 오케스트레이션, 자율 진행")** — main은 b5ppl 머지(`71c2936`) 위다. 머지 파동마다 푸시한다(아래 「결정」 ①). 끝난 파동, 그 사이의 리드 몫, 결정 원문은 장부 「「지금」에서 옮긴 것」에 있다.

- **비행 중(전부 opus)**: `b4attn` ‖ `b4engram`(키 쪽과 쿼리 쪽을 둘로 — wkv와 키 norm은 0층 그늘, 쿼리 쪽은 1·14층 임계) ‖ 13:30 `b4woa`(묶음 `wo_a`를 `q8_0_gemv_heads` 한 런치로) ‖ `harness`(「GPU 선 트리아지」 ①의 하니스 몫 — 매니페스트 한 번 읽기·색인·헤더·f16 마스크 행·스텝 세트 표) ‖ 13:35 `b4comp`(압축기 D + 인덱스 키 E) ‖ `tools2`(「GPU 선 트리아지」 ② — lease·witness 주인 하나, `prompt()` 한 벌, 락 없는 호스트 bin 러너, `ptx-scan` 피처 인자, 판정 비교·`sass_inflight`·두 트리 A/B를 상설 도구로) ‖ 14:03 `b5load`(V4.1 body 뼈대·스텝 이미지·`Split`을 받는 적재 이음매·`AnyEngine`을 gpu-gates로·게이트 배치) ‖ `b5host`(V4.1 호스트 expert 티어 — 384 expert, 샤드 짝을 타입으로, 클램프, q5_K, 할당 없는 진입점) ‖ 14:10 `ubsplit`(아래 b5ppl의 발견을 가린다 — 경계 뒤에 몰리면 상태 결함, 고르면 커널 경로; 위치별 KLD 도구 `kld_diff`는 B5의 비교 도구가 된다). 스펙은 세션 스크래치 `wave-m3/spec-*.md`. **보고를 받고 피드백 한 번 중**: `b4hc`(보고: 여섯 세트 80 서브층 판정 991줄 PASS — `q3k_sb_decode`를 크레이트 밖으로 열며 안전한 `pub fn`이 됐다, R1로 `unsafe fn`에) ‖ `b4moe`(보고: 637사이트 PASS — b11c 전 base라 q8_0 평면을 f16 비트로 옮기고, 레인 함수 둘의 R1, `Q8Act` 필드 접근자).
...
- **다음(리드)**: 보고를 받는 대로 검수·머지한다. b4hc·b4moe는 피드백 라운드가 끝나면 머지한다. b4hc를 머지할 때 `bench_v41`의 hc 행 경보를 처리한다: Q8Act 상한이 20,480으로 오르면 벤치가 일부러 빨강을 낸다. hc 투영은 이제 `ds41_hc_pre`(분할 K q3_K + HC_PRE 융합)가 맡으므로, `gpu` 피처 벤치의 일반 q3_K gemv로 재면 다른 커널을 재게 된다. ~~1단계용 스텝 세트 `d1n` 덤프~~ 떴다(`d90de1a`, 876 M). 테스트 env ①의 나머지(①b — 경로 사본 여섯·`ref_dir`의 `Arch`)는 harness 뒤. 도구 ②는 b2b의 A6000 A/B 뒤(A/B에 쓸 측정 러너를 만진다). 테스트 env 라운드와 도구 라운드(「GPU 선 트리아지」 ①·②)는 둘째 파동과 파일이 겹치지 않으면 띄운다. 사용자 판단 대기: IOMMU를 pt로 바꾸는 재부팅(GDS의 다음 단계). 조용한 틈에 engram 임대 측정과 `measure-qdot-rate`.
...
- b5meta: 모델 시험 크레이트에 매니페스트 파서가 둘이다(`tests/ds41_meta.rs`의 `Manifest`, `tests/common/oracle.rs`의 `Oracle`) — 세트 이름을 받는 읽기 함수 하나를 `tests/common`에(R14, ~60줄).
...
- tools2 뒤 잔여(b5ppl): `ik-ppl.sh`의 `KLD_VOCAB` 리터럴과 `TEXT` 경로를 모델 프로필(`tools/ref/models/deepseek41.sh`) 속성으로(XS). `ik-ppl.sh`의 `witness()` 사본은 tools2가 일부러 남긴 하나다 — 다음 도구 라운드에서 공용 라이브러리로.
...
- 락 없는 호스트 bin 러너 하나(b4plan2 ④-7): `gate-ds41-plan`과 `bench-v41-host`는 `gate.sh && cargo build && timeout <bin>`으로 이어져 종료 코드의 주인이 둘이다 — `tools/gpu-gate.sh`의 락 없는 짝으로(S). `tools/ptx-scan.sh:24-25`의 `router_shape` 설명에 `q8_0_gemv_heads`가 빠졌다(b11c, XS). `just ptx-scan`이 `--features gpu`로 고정돼 V4.1 빈(`--features deepseek41`)을 못 스캔한다 — FEATURES 인자(b4rope, S).
...
- **heads 커널과 q8_0 형상 레버**(b11c 감사, [유도]): `model/kernels.rs:112`의 `row / rows_per_head`가 첫 적재 앞에서 21명령짜리 32비트 나눗셈 에뮬레이션이다 — 2-D 격자(`blockIdx.y` = 헤드)나 시프트로 빼면 비트가 같다(S; `wo_a`를 이 커널로 돌리는 b4woa에서도 rows_per_head 1024라 같은 자리다). 작은 격자 사이트(attn_kv 0.19파동·attn_output_a 0.38·attn_q_a 0.48)를 그래프 병렬 가지로(attn_q_a ∥ attn_kv, 상한 −0.32 ms, M). 작은 격자 split-K(float 순서 클래스, σ·e2e 핀 검토, M). heads K ≤ 128에서 warp당 4행(float 순서, M). int8 활성 + dp4a(정확도 클래스 — `exact_ref --act` 시뮬과 비트 게이트 재핀, L). b11c가 잰 q8_0 몫: 토큰 그래프 20.873 ms 중 ≈ 13.00 ms(대역 행 8.92 + 바닥 행 4.08)[유도] — 가장 큰 레버는 바이트가 아니라 런치 형상이다.
