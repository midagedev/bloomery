...
| 라운드 | 만드는 것 | 게이트 | 의존 |
|---|---|---|---|
| 1-1 | GGUF 로더와 텐서 맵, 양자 타입별 디퀀트 | 파일의 텐서 377개가 전부 이름·형상·오프셋으로 풀린다. 타입마다 한 텐서를 디퀀트해 ggml `to_float`와 ≤ 1e-6 | 없음. GPU 불필요 |
| 1-2 | 임베딩 + 블록 0(MLA 어텐션 + dense FFN), CPU, f32, M=1 | 블록 0 출력이 ik의 중간 텐서와 ≤ 1e-3 | 1-1 |
| 1-3 | MoE 블록: 라우터, top-6, expert 디스패치. `q3k-cpu` 커널이 첫 호출자를 얻는다 | 블록 N 출력이 ik와 ≤ 1e-3 | 1-2 |
...
1-1과 1-2는 파일이 겹치지 않으므로 병렬로 돌린다. 나머지는 직렬이다.
