...
| 26 (09-26 오후~) | `hoststream`(M2b 통과·예산 해결 시 — 사다리 5: 호스트 h = 0.21 / PCIe 분담, P = 4096 층당 757 → 160 ms[유도], L) ‖ `uniongroup`(M) ‖ `h3tile`(4행 레지스터 타일, M–L) ‖ `slotsnap`(M–L) ‖ ~~`fixup5`(트래커 10–15건)~~ 착륙 `7928a0f`(09-25 밤) | 시팅 10(밤, 승인), 최종 숫자표 시팅(네 축 + 참조, 한 표) | 09-27 글·영상·스크럽 → 공개 후보(숫자만 09-27, M2 포함 09-28) |
