...
| A3 | 27층 + lm_head + argmax를 한 그래프(또는 층 그래프의 사슬)로, 토큰 루프 | 고정 프롬프트 32개에서 ik CUDA와 greedy 토큰 비교(발산 집합 핀), **첫 tok/s 대 ik 216.6**(깊이 0) — **첫 라운드 머지(e2e 0294d4a, 2026-09-21)**: 1그래프 755노드, **198.7 tok/s = 0.918×**(graph, 3090, n=2, SMOKE); 남은 것 = 발산 집합 핀(참조 파일이 `kv_cache_clear` 오염이라 재생성 먼저), 프롬프트 33개 |
