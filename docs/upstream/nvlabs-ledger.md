...
| 4 | cuda-oxide `cuda-macros` | `launch_contract`의 `requires` 산술이 `+ - *`만 받는다(나눗셈 불가) — `qs.len() >= n_rows * k / 4` 같은 워드·블록 단위 계약을 `4 * qs.len() >= n_rows * k`로 돌려 써야 하고 그 형태는 `32 \| k`일 때만 동치다 | 후보(기능 요청급, 낮음) — P3 위임 라운드가 읽음(`cuda-macros/src/cuda_module/contract.rs:461, 757-763`), 리드 미대조 | `crates/gpu/src/q8f32.rs`의 `q8_0_gemv` 계약 |
