...
| 2 | cutile-rs `cuda-bindings` 0.3.1 | `cuGraphInstantiate`·`cuGraphLaunch`·`cuGraphExecDestroy`·`cuGraphDestroy` 바인딩 부재. `cuda-core`의 `CudaStream::begin_capture`/`end_capture`는 `CUgraph`를 돌려주지만 그것으로 할 수 있는 일이 없다 — 캡처는 있고 재생이 없다 | 확인은 **crates.io 0.3.1까지**(`grep -rn "fn cuGraph"` 0건, 최신 릴리스도 0.3.1). 업스트림 HEAD에 이미 들어갔는지는 미확인 — 제출 전 클론해서 grep(raw 페치 한 번은 0건이었지만 404도 0건이다). 미제출. bloomery가 `extern "C"`로 직접 선언해 쓰면 그 코드가 PR 본체가 된다. **함정**(박스 `cuda.h` 13.0에서 읽음): libcuda가 내보내는 `cuGraphInstantiate` 심볼은 5인자 구식(`phErrorNode, logBuffer, bufferSize`)이고, 3인자 `(exec, graph, flags)`는 `cuGraphInstantiateWithFlags`다(헤더가 `#define cuGraphInstantiate cuGraphInstantiateWithFlags`). 3인자 시그니처를 맨 이름으로 선언하면 5인자 심볼에 링크돼 포인터 둘에 쓰레기가 간다 → `#[link_name = "cuGraphInstantiateWithFlags"]`. `cuGraphLaunch`·`cuStreamEndCapture`는 cuda-core 관례대로 `_ptsz` 없는 맨 이름(cuda-core는 `cuStreamBeginCapture_v2`를 부른다) | `docs/gpu-design.md` §미정 |
