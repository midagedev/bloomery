...
같은 크레이트의 V2-Lite 게이트 전부가 함께 붉어지는데, 크레이트가 다르면 V4.1 빌드만 깨진다. 게이트 크레이트가 그 크레이트를 늘 들이면 경계가 도로 무너지므로, `bloomery-gpu-gates`는 V4.1 크레이트를
피처 `deepseek41`(V4.1 op 게이트)로만 들인다(2026-09-23, b4infra 보고). 이것이 이 설계에서
...
- `tools/check-arch.sh`(맥, grep뿐 — `check-recipes.sh`와 같은 부류, `just check-arch`):
  아키텍처 목록은 손으로 적지 않는다: `crates/*/src/arch/<이름>/` 디렉터리가 목록이다(2026-09-24 — 그 전에는
  `deepseek2|deepseek41`로 고정돼 `qwen3moe::` 경로가 조용히 통과했다).
  ① 아키텍처 디렉터리끼리 서로를 `use`하지 않는다
  ② `blk.N.<name>` 문자열 리터럴과 `<아키텍처>.` 접두 키는 `arch/`와 `tools/ref/models/` 밖에 없다.
  ...
  ④ 공유 파일(`crates/*/src` 가운데 `arch/` 밖)은 아키텍처 모듈 경로(`deepseek2::`·`deepseek41::`)를 쓰지 않는다.
     ...
     잇는 디스패치 지점(`Deepseek2Model`·`Qwen3moeModel` 별칭, `bloomery-decode`, 오라클 표의 `for_arch`, deepseek41v 탑 자신의
     크레이트인 `gpu-vision`의 인코더 `use`)과 게이트 하네스의 기본
...
- 이관 라운드의 증명은 전부 「Derive first」의 **이동 클래스**다: `just ptx-scan` 표 동일(`gate_p5`·`gate_p8` ~~54행~~ 엔트리 53행 — 54는 b6 전에 배너·머리줄까지 센 줄 수였다),
