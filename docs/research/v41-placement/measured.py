...
# M: `just time-gpu-v41`, A6000, after B11 (lead ABAB 09-23, B arms; graph_us_mean per site x launches)
TOKEN_TODAY_MS = 22.610      # token graph, 784 nodes (grouped wo_a as 8 launches per layer)
TOKEN_WOA_MS = 20.970        # same bytes with wo_a as one launch per layer, 504 nodes
SEL_MS = 6.389               # q3k/q4k `_sel`, 6 slots x 38 layers in the token graph
SHARED_MS = 2.990            # shared expert gate/up/down, 40 layers
...
HOST_A, GPU_A = 3.409e9, 0.634e9            # placement (a): prefix n_l = 63-64 per layer
...
B11B = (0.0, 1.7)            # D: prize <= 1.7 ms at 700 GB/s
B11B_SHARED = 0.43           # D: the shared expert's part of that prize, hidden under the host leg in R1
...
def compose(dense, host_bytes, gpu_bytes, gbps, nongemv, join, b11b):
    ...
    hidden = SHARED_MS + gexp - (B11B_SHARED * b11b / B11B[1] if b11b else 0.0)
