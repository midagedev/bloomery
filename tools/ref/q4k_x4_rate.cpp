// MUL-27 rate reference: ik's OWN x4 kernel over the same synthetic shape
// qdot-rate benches (360,448 rows of K=2048 Q4_K, one quantized column,
// single thread, 6 passes) — the number the Rust kernel must beat, measured
// on this box rather than quoted. Same kernel-table entry as q4k_x4_ref.cpp
...
int main() {
    const size_t rs = 144 * (kK / 256);

    // Same bytes as qdot-rate's xorshift64* filler: any bytes are valid Q4_K codes,
    ...
    if (!iqk_set_kernels_kquants(kK, GGML_TYPE_Q4_K, GGML_TYPE_Q8_2_X4, kernels, func16) || !kernels[0]) {
        fprintf(stderr, "ik declined the Q4_K x Q8_2_X4 pairing\n");
        return 1;
    }
    ...
    printf("ik Q4_K x4 rows %d x %zu B x %d passes in %.3f s = %.1f GB/s (dst[0] %g)\n",
