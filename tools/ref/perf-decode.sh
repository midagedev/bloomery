...
lease_take
witness pre
"$BIN" -m "$MODEL" --tokens "$TOKENS" -n "$N" > "$OUT/decode.log" 2>&1 &
pid=$!
...
perf record -q -e cpu-clock -F 10000 "${target[@]}" -o "$OUT/perf.data" -- sleep "$SECS" 2> "$OUT/perf.err" || echo "[perf] record rc=$?: $(tr '\n' ' ' < "$OUT/perf.err")"
...
perf script -i "$OUT/perf.data" -F tid,sym 2> /dev/null | awk -v main="$pid" '{print ($1 == main ? "main" : "workers"), $2}' \
...
for sym in ${PERF_ANNOTATE:-}; do
  ...
  perf annotate -i "$OUT/perf.data" --stdio -s "$sym" 2> /dev/null \
    | grep -E '^ +[0-9]+\.[0-9]+ +:' | sort -rn | head -n 12 || true
done
for sym in ${PERF_CALLERS:-}; do
  echo
  echo "=== main thread, callers of $sym ==="
  perf report -i "$OUT/perf.data" --stdio --no-children --percentage relative -S "$sym" -G 2> /dev/null \
