// Rate reference: ik's OWN Q5_K x4 kernel over the same synthetic shape
// qdot-rate benches for Q5_K (360,448 rows of K=2304 — the V4.1 ffn_down_exps
// row length — one quantized column, single thread, 6 passes): the number the
// Rust kernel must beat, measured on this box rather than quoted. Same
// kernel-table entry as q5k_x4_ref.cpp (mul_mat_qX_K_q8_2_X4_T<DequantizerQ5K_AVX2, 1>).
#include "ggml.h"
...
static const int kK = 2304;
static const int kRows = 360448;
static const int kPasses = 6;

int main() {
    const size_t rs = 176 * (kK / 256);

    // Same bytes as qdot-rate's xorshift64* filler: any bytes are valid Q5_K codes,
    ...
    if (!iqk_set_kernels_kquants(kK, GGML_TYPE_Q5_K, GGML_TYPE_Q8_2_X4, kernels, func16) || !kernels[0]) {
        fprintf(stderr, "ik declined the Q5_K x Q8_2_X4 pairing\n");
        return 1;
    }
    ...
    printf("ik Q5_K x4 rows %d x %zu B x %d passes in %.3f s = %.1f GB/s (dst[0] %g)\n",
