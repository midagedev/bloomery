// MUL-32 rate reference: ik's OWN Q5_0 kernel over the same synthetic shape
// qdot-rate benches (360,448 rows of K=1408 Q5_0 — the ffn_down_exps shape,
// one quantized column, single thread, 6 passes) — the number the Rust
// kernel must be judged against, measured on this box rather than quoted.
// Same kernel-table entry as q5f0_ref.cpp
// (mul_mat_qX_1_q8_2_T<Q5_0_1_Unpacker, 1>).
#include "ggml.h"
...
static const int kK = 1408; // moe_intermediate: 44 x 32-value blocks, 11 x4 groups
static const int kRows = 360448;
static const int kPasses = 6;

int main() {
    const size_t rs = 22 * (kK / 32); // 968

    // Same bytes as qdot-rate's xorshift64* filler: any bytes are valid Q5_0 codes,
    ...
    if (!iqk_set_kernels_legacy_quants(kK, GGML_TYPE_Q5_0, GGML_TYPE_Q8_2_X4, kernels, func16) || !kernels[0]) {
        fprintf(stderr, "ik declined the Q5_0 x Q8_2_X4 pairing\n");
        return 1;
    }
    ...
    printf("ik Q5_0 x4 rows %d x %zu B x %d passes in %.3f s = %.1f GB/s (dst[0] %g)\n",
