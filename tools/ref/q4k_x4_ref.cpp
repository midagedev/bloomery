...
int main() {
    ...
    const std::string out_p = std::string(kDataDir) + "/ref/q4k-x4-ik-dot.txt";
    const std::string tmp_p = out_p + ".tmp." + std::to_string(getpid());
    FILE *out = fopen(tmp_p.c_str(), "w");
    if (!out) {
        fprintf(stderr, "q4k_x4_ref: cannot open %s for writing: %s\n", tmp_p.c_str(), strerror(errno));
        return 1;
    }
    ...
    if (fclose(out) != 0) {
        fprintf(stderr, "q4k_x4_ref: cannot write %s: %s\n", tmp_p.c_str(), strerror(errno));
        return 1;
    }
    if (rename(tmp_p.c_str(), out_p.c_str()) != 0) {
        fprintf(stderr, "q4k_x4_ref: cannot rename %s to %s: %s\n", tmp_p.c_str(), out_p.c_str(), strerror(errno));
        return 1;
    }
