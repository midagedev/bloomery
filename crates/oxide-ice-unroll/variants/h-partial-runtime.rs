...
mod kernels {
    ...
    pub fn ice(mut out: DisjointSlice<u32>, n: usize) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    module.ice(&stream, &prepared, &mut out_dev, 4)?;

    let out_host = out_dev.to_host_vec(&stream)?;
    let errors = out_host.iter().filter(|&v| *v != 8).count();
    if errors == 0 {
        println!("PASSED: all {} elements == 8", N);
    } else {
