...
mod kernels {
    ...
    pub fn ice(w: &[u32], mut out: DisjointSlice<u32>) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    const N: usize = 32;
    let w_host: Vec<u32> = vec![10, 20, 40, 80];
    ...
    module.ice(&stream, &prepared, &w_dev, &mut out_dev)?;

    let out_host = out_dev.to_host_vec(&stream)?;
    let errors = out_host.iter().filter(|&v| *v != 80).count();
    if errors == 0 {
        println!("PASSED: all {} elements == 80", N);
    } else {
