...
|---|---:|---:|---:|---:|---:|---:|---:|
| mulle | 1 | 140.37 | 1.000 | 79.05 | 1.000 | 207.37 | 1.000 |
| mulle | 2 | 170.70 | 1.216 | 128.77 | 1.629 | 231.54 | 1.117 |
| mulle | 3 | 200.63 | 1.430 | 183.56 | 2.323 | 280.30 | 1.352 |
| mulle | 4 | 243.75 | 1.736 | 220.96 | 2.795 | 324.58 | 1.565 |
| mulle | 6 | 325.83 | 2.321 | 308.25 | 3.899 | 413.73 | 1.995 |
| mulle | 8 | 404.25 | 2.880 | 400.76 | 5.070 | 499.96 | 2.411 |
...
(existing m1/m8 rows, same runs: mulle stack 141.24→409.82 = 2.902,
...
than a weight re-read (µs/col between consecutive M grows), and mulle's
curves rise much faster than ggml's except on head. Slope of the k-family,
µs per extra column (endpoint (t₈−t₁)/7): mulle 37.70 / 45.96 / 41.80,
ggml 33.54 / 27.84 / 38.64. Normalized per warp (slope/N): mulle 0.419 / 0.832 / 0.408 ns per
warp-column — Q4_K exactly 2× Q3_K. Per-token amortization t(M)/(M·t(M=1))
at M=8: mulle 0.360 / 0.634 / 0.301 vs ggml 0.241 / 0.408 / 0.270 — the
q8_1 activation is amortized far better in ggml's layout; mulle's M=8 was
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| mulle | 1 | 147.79 | 536.6 | 1.000 | 1.000 | 79.13 | 805.0 | 1.000 | 1.000 | 207.21 | 830.3 | 1.000 | 1.000 |
| mulle | 2 | 159.30 | 497.8 | 1.078 | 0.539 | 82.22 | 774.8 | 1.039 | 0.520 | 216.36 | 795.1 | 1.044 | 0.522 |
| mulle | 3 | 171.88 | 461.4 | 1.163 | 0.388 | 97.89 | 650.7 | 1.237 | 0.412 | 242.55 | 709.3 | 1.170 | 0.390 |
| mulle | 4 | 189.13 | 419.3 | 1.280 | 0.320 | 117.33 | 542.9 | 1.483 | 0.371 | 275.88 | 623.6 | 1.331 | 0.333 |
| mulle | 6 | 220.98 | 358.8 | 1.495 | 0.249 | 156.91 | 406.0 | 1.983 | 0.331 | 341.25 | 504.1 | 1.647 | 0.275 |
| mulle | 8 | 257.79 | 307.6 | 1.744 | 0.218 | 191.93 | 331.9 | 2.425 | 0.303 | 402.30 | 427.6 | 1.942 | 0.243 |
...
(existing m1/m8 rows: mulle stack 137.19→255.51 = 1.863, attnstk
78.54→183.34 = 2.334, head 204.66→406.62 = 1.987.)

After the levers mulle's M=8 is faster than ggml's in absolute time on all
...
## Gates (in-process, same-invocation anchors; pre = Part A run, post = both final runs)
    ...
    pre : gate2 stack_m8/stack_m1: mulle 2.902 vs ggml 1.998 -> FAIL
    pre : gate2 stack_k8/stack_k1: mulle 2.880 vs ggml 1.930 -> FAIL
    pre : gate2 attnstk_m8/attnstk_m1: mulle 5.145 vs ggml 3.320 -> FAIL
    pre : gate2 attnstk_k8/attnstk_k1: mulle 5.070 vs ggml 3.264 -> FAIL
    pre : gate2 head_m8/head_m1: mulle 2.530 vs ggml 2.200 -> info
    pre : gate2 head_k8/head_k1: mulle 2.411 vs ggml 2.160 -> info
    pre : gate3 stack_m1: mulle 561.45 GB/s vs floor 298.46 (0.9 x ggml 331.62) -> PASS
    pre : gate3 attnstk_m1: mulle 806.62 GB/s vs floor 690.51 (0.9 x ggml 767.23) -> PASS

    post (run 1): gate2 stack_m8/stack_m1: mulle 1.863 vs ggml 2.011 -> PASS
    post (run 1): gate2 stack_k8/stack_k1: mulle 1.744 vs ggml 1.912 -> PASS
    post (run 1): gate2 attnstk_m8/attnstk_m1: mulle 2.334 vs ggml 3.233 -> PASS
    post (run 1): gate2 attnstk_k8/attnstk_k1: mulle 2.425 vs ggml 3.289 -> PASS
    post (run 1): gate2 head_m8/head_m1: mulle 1.987 vs ggml 2.261 -> info
    post (run 1): gate2 head_k8/head_k1: mulle 1.942 vs ggml 2.145 -> info
    post (run 1): gate3 stack_m1: mulle 578.04 GB/s vs floor 299.72 (0.9 x ggml 333.02) -> PASS
    post (run 1): gate3 attnstk_m1: mulle 811.08 GB/s vs floor 660.43 (0.9 x ggml 733.81) -> PASS

    post (run 2): gate2 stack_m8/stack_m1: mulle 1.872 vs ggml 2.013 -> PASS
    post (run 2): gate2 stack_k8/stack_k1: mulle 1.719 vs ggml 1.932 -> PASS
    post (run 2): gate2 attnstk_m8/attnstk_m1: mulle 2.347 vs ggml 3.259 -> PASS
    post (run 2): gate2 attnstk_k8/attnstk_k1: mulle 2.388 vs ggml 3.335 -> PASS
    post (run 2): gate2 head_m8/head_m1: mulle 1.999 vs ggml 2.208 -> info
    post (run 2): gate2 head_k8/head_k1: mulle 1.948 vs ggml 2.155 -> info
    post (run 2): gate3 stack_m1: mulle 581.27 GB/s vs floor 299.23 (0.9 x ggml 332.48) -> PASS
    post (run 2): gate3 attnstk_m1: mulle 811.10 GB/s vs floor 678.30 (0.9 x ggml 753.67) -> PASS
...
27 of 28 mulle rows agree within 1.6% (gate anchors all ≤ 1.6%), verdicts
...
Ambient note: mulle stack_m1 here is 136.4-137.2 µs vs the 127.9 µs
...
throughout; never touched. A parallel round (mulle-attn) may run cargo test
on the same box — loadavg ~0.4-0.9 with io-pressure ≤ 0.14 avg10 shows it
never overlapped a timed section.

## Reproduce

    MULLE_DATA=/root/mulle-data-amort ./tools/box.sh 'bash tools/ref/measure.sh'

Builds the ref, regenerates `$MULLE_DATA` (28 y_ref rows + ggml_timings.txt),
