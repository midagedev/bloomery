...
//! B5 phase 1: positions below 512, no selection), on the gate placement
...
//!   step4 set's injected state is bit-identical to an eager run of it (the
//!   logits and every cache, compressed row, index key and compressor state).
...
mod gate {
    ...
    fn parse_args() -> Result<Args, GateError> {
        const USAGE: &str =
            "usage: gate_deepseek41_step [--structure] [--sets] [--greedy PROMPT] [--ppl TAG]";
        let mut a = Args {
            structure: false,
            sets: false,
            greedy: None,
        ...
        if !(a.structure || a.sets || a.greedy.is_some() || a.ppl.is_some()) {
    ...
    pub fn run() -> Result<(), GateError> {
        ...
        if args.sets {
            pass &= sets(&mut m, &mut head, &split, &hp)?;
        }
    ...
    /// One decode-step set: its manifest, the step, and the host plan of it.
    struct Set {
        name: &'static str,
        man: RefManifest,
        pos: u32,
    ...
    fn set_state(set: &Set, hp: &Hparams) -> Result<Vec<LayerState>, GateError> {
        ...
        for (l, kind) in hp.layers.iter().enumerate() {
            ...
            let (rows, comp) = match (kind.stream, kind.compressor) {
                (Some(st), Some(c)) if st.kv_source == l => {
                    ...
                    let pre = widened_f16_rows_in(&man.dir, man.input(leaf, 0)?)?;
            ...
            out.push(LayerState { ring, rows, comp });
        }
        Ok(out)
    }
    ...
    /// or its row at ratio 1, and three for index keys) and the MoE
    ...
    fn collect(out: &mut Vec<SeamOut>) -> impl FnMut(&Gpu, Seam<'_>) -> Result<(), GpuError> + '_ {
    ...
    fn sets(
        ...
        for name in SETS {
            ...
            run_step(gpu, w, body, head, &set, &mut collect(&mut seams))?;
    ...
    /// through `comb` (the streams' spread `κ`) and `post` (`g`).
    fn term(set: &Set, hp: &Hparams, stage: Stage, l: usize) -> Result<(f64, f64), GateError> {
        let man = &set.man;
        ...
        match stage {
            Stage::Attn => {
                ...
                v += (band * f64::from(amax) / (BAND_SIGMAS * rms)).powi(2);
    ...
    fn check_seams(
        ...
        for o in seams {
            ...
            let (body_term, mix_term) = term(set, hp, o.stage, o.layer)?;
