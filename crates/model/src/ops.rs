...
fn matmul_q_multi(
    ...
    // f32 round trip. The k check is the WEIGHT TYPE'S block contract, owned
    // by `qdot::k_granularity` so this call site and the kernel's own shape
    // validation cannot drift apart: 256-value super-blocks for Q3_K/Q4_K,
    // 32-value blocks for Q5_0. MUL-32 (2026-09-20): Q5_0's rows in this
    // model are the ffn_down_exps k = 1408 = 44 x 32 — NOT a multiple of
    // 256 — so the single 256-value check this line carried until now would
    // never fire on the stage table's largest site (Q5_0-batch, 23.1 ms/step
    // = 32.5% of wall at the MUL-27 measurement). The per-type number ADDS
    // Q5_0's contract; Q3_K/Q4_K keep theirs exactly (256) and their
    // kernels' shapes are untouched. Decided once for the whole batch; every
